...
export class createTable1638322857195 implements MigrationInterface {
    name = 'createTable1638322857195'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`db\`.\`imageProduct\` (\`id\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`is_main\` tinyint NOT NULL, \`product_fk_id\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`db\`.\`product\` (\`id\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`detail\` varchar(255) NOT NULL, \`price\` double NOT NULL, \`category_fk_id\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`db\`.\`imageProduct\` ADD CONSTRAINT \`FK_952bf92c747ec7c0965050ab77d\` FOREIGN KEY (\`product_fk_id\`) REFERENCES \`db\`.\`product\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
